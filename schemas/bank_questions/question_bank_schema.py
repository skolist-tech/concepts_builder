...
class SolvedExample(BaseModel):
    """This class represents a question in the question bank."""
    # Question Text Related
    question_text: str = Field(default=None, description="The question text, formatted properly in latex wherever needed")
    ...
    match_columns: Optional[List[MatchColumn]] = Field(
        default=None, description="List of columns for 'Match The Following' type questions"
    )
    ...
    correct_mcq_option: Optional[int] = Field(
        default=None, description="Correct option (1-4) (if applicable in MCQ)"
    )
    msq_option1_answer: Optional[bool] = Field(
        default=None, description="Is option 1 correct (if applicable in MSQ)"
    )
    msq_option2_answer: Optional[bool] = Field(
        default=None, description="Is option 2 correct (if applicable in MSQ)"
    )
    msq_option3_answer: Optional[bool] = Field(
        default=None, description="Is option 3 correct (if applicable in MSQ)"
    )
    msq_option4_answer: Optional[bool] = Field(
        default=None, description="Is option 4 correct (if applicable in MSQ)"
    )
    istrue: Optional[bool] = Field(
        default=None, description="True/False answer (if applicable in True/False questions)"
    )
    ...
    question_type : Literal[
        ...
        "FillInTheBlank",
        "TrueFalse",
        "MathTheFollowing"] = Field(
        default=None, description="Type of question, to be strictly one of MCQ4, MSQ4, Short Answer, Long Answer, FillInTheBlank, TrueFalse, MathTheFollowing"
    )
    concepts : List[str] = Field(
        default=[], description="List of concepts involved in the question, map to the concepts list provided separately"
    )
    is_image_needed: Optional[int] = Field(
        default=0, description="1 if figure or image is referenced or needed, else 0"
    )

class SolvedExamplesBank(BaseModel):
    """This Question Bank Represents collection of all solved examples from a chapter"""
...
class ExerciseQuestion(BaseModel):
    """This class represents an exercise question in the question bank."""
    # Question Text Related
    question_text: str = Field(default=None, description="The question text, formatted properly in latex wherever needed")
...
class ExerciseQuestionsBank(BaseModel):
    """This Question Bank Represents collection of all exercise questions from a chapter"""
    chapter_name: str = Field(..., description="Name of the chapter")
