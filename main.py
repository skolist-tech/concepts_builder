...
from schemas.bank_questions.exercise_bank_to_sql import upload_exercise_bank_to_supabase
...
async def main8():
    """
    Generate and upload exercise questions and exercise question concept maps 
    from the exercise questions JSON files directly to Supabase using async processing.
    """
    # Create Supabase client once
    supabase_url = os.getenv("SUPABASE_URL")
    supabase_key = os.getenv("SUPABASE_SERVICE_KEY")
    ...
    async def process_chapter(chapter_pdf_path: str) -> tuple[str, dict]:
        ...
        # Path to the exercise questions JSON file
        exercise_questions_json_path = os.path.join(
            CSV_OUTPUT_FOLDER_PATH,
            chapter_name + "_exercise_questions.json"
        )
        
        # Check if the exercise questions JSON file exists
        if not os.path.exists(exercise_questions_json_path):
            logger.warning(f"Exercise questions JSON not found for {chapter_name} at {exercise_questions_json_path}. Skipping.")
            return chapter_name, {}
        
        try:
            # Load the exercise bank from JSON (run in thread pool)
            logger.info(f"Loading exercise questions from: {exercise_questions_json_path}")
            
            def _load_json():
                return load_exercise_bank_json(exercise_questions_json_path)
            
            exercise_bank = await asyncio.get_event_loop().run_in_executor(None, _load_json)
            
            # Upload to Supabase
            logger.info(f"Processing chapter: {chapter_name}...")
            stats = await upload_exercise_bank_to_supabase(
                client=client,
    ...
    logger.info(f"Starting concurrent processing of {len(chapter_pdf_path_list)} chapters (limit 1)")
    tasks = [process_with_semaphore(chapter_pdf_path) for chapter_pdf_path in chapter_pdf_path_list]
    results = await asyncio.gather(*tasks, return_exceptions=True)
