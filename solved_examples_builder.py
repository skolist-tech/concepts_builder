#!/usr/bin/env python
"""
Solved Examples Builder CLI

Extracts solved examples from chapter PDFs and saves to JSON with UUIDs.

Usage:
    python solved_examples_builder.py --input_dir <subject_directory_path> \
        --output_dir <solved_examples_json_output_directory_path> \
...
def get_output_json_path(pdf_path: Path, output_dir: Path) -> Path:
    ...
    stem = pdf_path.stem
    return output_dir / f"{stem}_solved_examples.json"
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract solved examples from chapter PDFs and save to JSON with UUIDs"
    )
    parser.add_argument(
        "--input_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Output directory for solved examples JSONs (should also contain concept CSVs)"
    )
    parser.add_argument(
        "--subject_id",
        type=str,
