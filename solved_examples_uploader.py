#!/usr/bin/env python
"""
Solved Examples Uploader CLI

Uploads solved examples JSONs (with UUIDs) to Supabase.
...
def get_json_files(input_dir: Path) -> List[Path]:
    """Get all solved examples JSON files from the input directory."""
    if not input_dir.exists():
        raise FileNotFoundError(f"Input directory not found: {input_dir}")
    
    json_files = sorted(input_dir.glob("*_solved_examples.json"))
    if not json_files:
        raise ValueError(f"No solved examples JSON files found in: {input_dir}")
...
def question_to_db_record(
    ...
    if not question_id:
        raise ValueError("Question missing 'id' field - regenerate JSON with solved_examples_builder")
...
async def upload_solved_examples_from_json(
    ...
    """
    Upload solved examples from a JSON file to Supabase.
    
    Reads UUIDs from the JSON file (question IDs already embedded).
    
    Args:
        client: Supabase client instance.
        json_path: Path to the solved examples JSON file.
    
    Returns:
        Statistics dictionary with upload results.
    """
    logger.info(f"Loading solved examples from JSON: {json_path}")
    ...
    questions = data.get("solved_examples_questions", [])
...
async def upload_all_solved_examples(input_dir: Path) -> None:
    """Upload all solved examples JSONs from the input directory."""
    ...
    for i, json_path in enumerate(json_files, 1):
        ...
        try:
            stats = await upload_solved_examples_from_json(client, json_path)
            ...
            chapter_name = json_path.stem.replace("_solved_examples", "").lstrip("0123456789_")
            chapter_results.append({
                "name": chapter_name,
                "questions": 0,
                "concept_maps": 0,
...
def main():
    parser = argparse.ArgumentParser(
        description="Upload solved examples JSONs (with UUIDs) to Supabase"
    )
    parser.add_argument(
        "--input_dir",
        type=str,
        required=True,
        help="Directory containing solved examples JSON files"
    )
    ...
    try:
        asyncio.run(upload_all_solved_examples(input_dir))
